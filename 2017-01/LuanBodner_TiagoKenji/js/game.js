...
class PlayState extends GameState {
    ...
    changeLevel() {
        ...
        if (Config.LEVEL <= 3) this.game.state.restart()
    ...
    takeScreenShot() {
        ...
        ServerComm.saveMedia(Config.ID, data, function(response) {
    ...
    loadMedia() {
        ...
        ServerComm.loadMedia(Config.ID, function(json) {
            ...
            for (i = 0; i < list.length; i++) {
                $('#div-screenshot').append(`<img src=${list[i]['src']} alt='game screenshot' class='screenshot' style='width: 48%; height: 48%'>`)
            }
        })
    }
    create() {
